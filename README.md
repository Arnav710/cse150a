...
We see that in terms of the models we built, the performace of the probability based agent was the best, followed by the similarity based agent and then the naive bayes agent. The tfidf + Naive Bayes approach was the most complicated among the three however it ended up performing the worst relative to the baseline. Before popular machine learning approaches, Naive Bayes was used for the purpose of retreival in addition to classification. This is why we tried the Naive Bayes approach. However, one potential reason for low performance could be that the tfidf vectors are very sparse. This is because the patient description only contains 5-15 words after preprocessing however the tfidf vectors are of length vocab_size (which is much larger). Further, the naive bayes independece relation of the words in a sentence being independent given the doctors response may be an oversimplification of the task. 
